...
from dbwarden.engine.core import Change, MigrationStatement, ObjectHandler, Op, RunPhase, StatementOrder
...
class CompositeTypeHandler(ObjectHandler):
    ...
    , **kwargs: Any) -> List[MigrationStatement]:
        from dbwarden.engine.core.plugin_api import qualified_name, quote_pg
        ...
        if op.object_type == "create_composite_type":
            ...
            col_defs = ", ".join(
                f"{quote_pg(c.get('name', ''))} {c.get('type', 'text')}"
                for c in columns
            )
